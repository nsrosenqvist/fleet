...
/// Build the argv for `podman exec -it [-e KEY=VAL]… <id> <cmd…>`.
/// Pure so the shape can be locked in by tests without spawning a
/// real podman subprocess. Env vars surface as `-e KEY=VAL` pairs
/// before the container id; empty env yields the bare attach argv.
fn attach_argv(
    container: &ContainerId,
    user_argv: &[String],
    env: &[(String, String)],
