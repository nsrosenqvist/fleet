...
fn build_workflow_run_command_with_detached_appends_flag() {
    ...
    let cmd = build_workflow_run_command(Path::new("/usr/local/bin/fleet"), "standard", None, true);
    let dbg = format!("{cmd:?}");
    assert!(dbg.contains("--detached"), "got: {dbg}");
}
...
fn orchestrator_row_label_marker_reflects_state() {
    assert!(
        orchestrator_row_label(&orchestrator("x", OrchestratorState::Detached)).starts_with('⏸')
    );
    assert!(orchestrator_row_label(&orchestrator("x", OrchestratorState::Closed)).starts_with('✗'));
}
