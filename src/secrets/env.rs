//! Environment-variable backend. Reads `$VAR` at fetch time. Useful
//! for CI runners where the token is already an env var of the host
//! process, and for explicit local overrides during testing.
...
mod tests {
    ...
    // Distinct names per test so concurrent tests can't race on the
    // same env var. Each name is unique to this module + test.
    ...
    fn fetches_when_set() {
        ...
        // to this test so concurrent tests cannot race.
        unsafe { std::env::set_var(SET_VAR, "secret-value-123") };
